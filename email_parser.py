...
output = pd.DataFrame(columns=["Email_id", "Location", "Event_Name", "Pizza_Status", "Event_Date", "Sent_Date", "Text"])
...
for message in mailbox.mbox('pizza!.mbox'):
    ...
    output = output.append(pd.DataFrame([[uuid.uuid4(), None, None, None, None, message['date'], prepros(content)]], columns=["Tweet_id", "Location", "Event_Name", "Pizza_Status", "Event_Date", "Sent_Date","Text"]))
